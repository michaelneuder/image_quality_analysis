...
def main():
    ...
    # reading in data from files. original and recon are combined
    ...
    for i in range(original_images_train.shape[0]):
        x_data_train.append(np.append(original_images_train[i], reconstructed_images_train[i]))
        y_data_train.append(comparison_images_train[i])
    for i in range(original_images_test.shape[0]):
        x_data_test.append(np.append(original_images_test[i], reconstructed_images_test[i]))
    ...
    weights = tf.Variable(tf.zeros([18432, 9216], tf.float32))
    ...
    acc = accuracy.eval(session=sess, feed_dict={x: x_data_test, y_:y_data_test})
    print(acc)
