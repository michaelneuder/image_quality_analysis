        ...
        for epoch_count in range(epochs):
            ...
            for i in epoch:
                ...
                for j in range(3):
                    ...
                    ax1.imshow(train_features[j,:,:,0], cmap='gray')
                    ax2.imshow(train_features[j,:,:,1], cmap='gray')
                    ax3.imshow(train_target[j,:,:,0], cmap='plasma')
                    ax4.imshow(prediction_ex[j,:,:,0], cmap='plasma')
                plt.savefig('predictions/single_batch/prediction_demo_batch{}.png'.format(batch_counter))
                plt.close()
                batch_counter+=1
