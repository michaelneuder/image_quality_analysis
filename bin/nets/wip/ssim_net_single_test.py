...
def main():
    ...
    target_data_train = np.reshape(comparison_images_train, [train_size, 86, 86, output_layer])
    target_data_test = np.reshape(comparison_images_test, [test_size, 86, 86, output_layer])
    ...
    xtest = tf.placeholder(tf.float32, [None, 96, 96, input_layer])
    ytest = tf.placeholder(tf.float32, [None, 86, 86, output_layer])

    # model
    prediction = conv_net(x, weights, biases)
    prediction_test = conv_net(xtest, weights, biases)
    ...
    cost_test = tf.reduce_mean(tf.square(tf.subtract(prediction_test, ytest)))
    ...
    with tf.Session() as sess:
        ...
        loss = sess.run(cost_test, feed_dict={xtest: test_data, ytest: target_data_test})
        print('loss on total set it {:.4f}'.format(loss))
